...
/**
 * （完结）
 * 1. 实现配置修改监听接口，最后调用onChange()
 * 2. 监听配置修改事件，最后调用onChange()
...
public class AutoUpdateConfigChangeListener implements ConfigChangeListener, ApplicationListener<ApolloConfigChangeEvent>, ApplicationContextAware {
    ...
    public AutoUpdateConfigChangeListener() {
        ...
        this.springConfigurationPropertyRegistry = SpringInjector.getInstance(SpringConfigurationPropertyRegistry.class);
        // ?? 数据模式
    ...
    public void onChange(ConfigChangeEvent changeEvent) {
        ...
        for (String key : keys) {
            ...
            // 1. check whether the changed key is relevant
            Map<String, WeakReference<Object>> targetConfigurationProperties = springConfigurationPropertyRegistry.get(beanFactory, key);
                ...
                Set<String> refreshedPrefix = new HashSet<>();
                 for (Map.Entry<String, WeakReference<Object>> val : targetConfigurationProperties.entrySet()) {
                     ...
                     if(!refreshedPrefix.contains(prefix)){
                         refreshConfigurationProperties(beanFactory, prefix, val.getValue().get());
                     }
    ...
    private void refreshConfigurationProperties(BeanFactory beanFactory,String prefix, Object bean) {
        try {
            springConfigurationPropertyRegistry.refresh(beanFactory, prefix, bean);
            logger.info("Auto update apollo changed configuration properties successfully, bean: {}", bean.getClass().getName());
        } catch (Throwable ex) {
            logger.error("Auto update apollo changed configuration properties failed, {}", bean.getClass().getName(), ex);
        }
    }
    ...
    private Object resolvePropertyValue(SpringValue springValue) {
        ...
        if (springValue.isJson()) {
            // 解析过程 略
            ApolloJsonValue apolloJsonValue = springValue.isField() ? springValue.getField().getAnnotation(ApolloJsonValue.class) : springValue.getMethodParameter().getMethodAnnotation(ApolloJsonValue.class);
